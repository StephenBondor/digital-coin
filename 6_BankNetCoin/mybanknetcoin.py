...
class Tx:
    ...
    def sign_input(self, index, private_key):
        signature = private_key.sign(spent_message(self, index))
        self.tx_ins[index].signature = signature

    def verify_input(self, index, public_key):
        tx_in = self.tx_ins[index]
        message = spent_message(self, index)
...
class Bank:
    ...
    def validate_tx(self, tx):
        ...
        for index, tx_in in enumerate(tx.tx_ins):  # loop over all inputs for the tx
            tx_out = self.utxo[tx_in.outpoint]
            tx.verify_input(index, tx_out.public_key)  # verify
    ...
    def fetch_utxo(self, pub_key):
        return [utxo for utxo in self.utxo.values() if utxo.public_key == pub_key]
